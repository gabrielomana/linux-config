...
#KDE PLASMA
echo "KDE PLASMA"
sudo apt install -y kde-plasma-desktop plasma-workspace-wayland --install-recommends
...
clear
echo "UNINSTALL"
sudo apt -y remove gwenview akregator kmail konversation krfb kmahjongg kmines dragonplayer elisa-player korganizer kontact kpat
sudo apt -y groupremove "LibreOffice"
...
sudo mv /etc/apt/trusted.gpg /etc/apt/trusted.gpg.d/keys_kubuntu.gpg
clear
sudo apt update -y
